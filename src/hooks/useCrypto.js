...
export const useCryptoPrices = (cryptoIds = 'bitcoin,ethereum,cardano,solana,avalanche-2,polkadot') => {
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      try {
        setLoading(true)
        const response = await axios.get(`${API_BASE}/prices?ids=${cryptoIds}`)
...
export const useMarketData = (perPage = 20, page = 1) => {
  ...
  useEffect(() => {
    const fetchMarketData = async () => {
      try {
        setLoading(true)
        const response = await axios.get(`${API_BASE}/markets?per_page=${perPage}&page=${page}`)
...
export const usePortfolio = (userId = 1) => {
  ...
  const fetchPortfolio = async () => {
    try {
      setLoading(true)
      const response = await axios.get(`${API_BASE}/portfolio?user_id=${userId}`)
...
export const usePriceHistory = (coinId, days = 7) => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        setLoading(true)
        const response = await axios.get(`${API_BASE}/history?coin_id=${coinId}&days=${days}`)
