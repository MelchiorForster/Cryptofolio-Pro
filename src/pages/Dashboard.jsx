...
import { TrendingUp, TrendingDown, DollarSign, Wallet, Bitcoin } from 'lucide-react'
import { useCryptoPrices, usePortfolio, calculatePortfolioValue, formatPrice, formatPercentage } from '../hooks/useCrypto'
...
const Dashboard = () => {
  ...
  const totalChange = Object.values(prices).reduce((sum, coin) => sum + (coin.eur_24h_change || 0), 0) / Object.keys(prices).length
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="card">
          ...
          <div className={`flex items-center mt-4 ${totalChange >= 0 ? 'text-crypto-green' : 'text-crypto-red'}`}>
      ...
      <div className="card">
        ...
        <div className="space-y-4">
          {portfolio.map((holding) => {
            ...
            const priceChange = prices[holding.coingecko_id]?.eur_24h_change || 0
            
            return (
              <div key={holding.id} className="flex items-center justify-between p-4 bg-crypto-dark rounded-lg">
                <div className="flex items-center space-x-4">
                  <div className="w-10 h-10 bg-crypto-accent rounded-full flex items-center justify-center">
                    <span className="text-crypto-dark font-bold text-sm">{holding.symbol}</span>
                  </div>
                  <div>
                    <p className="font-medium text-white">{holding.name}</p>
                    <p className="text-gray-400 text-sm">{Number(holding.amount).toFixed(4)} {holding.symbol}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-medium text-white">{formatPrice(holdingValue)}</p>
                  <p className={`text-sm ${priceChange >= 0 ? 'text-crypto-green' : 'text-crypto-red'}`}>
      ...
      <div className="card">
        ...
        <div className="space-y-4">
          {topGainers.map((coin) => (
            <div key={coin.symbol} className="flex items-center justify-between p-4 bg-crypto-dark rounded-lg">
              <div className="flex items-center space-x-4">
                <div className="w-10 h-10 bg-crypto-green/20 rounded-full flex items-center justify-center">
                  <span className="text-crypto-green font-bold text-sm">{coin.symbol}</span>
                </div>
                <div>
              ...
              <div className="text-right">
                ...
                <p className="text-crypto-green text-sm">{formatPercentage(coin.change)}</p>
              </div>
            </div>
          ))}
        </div>
